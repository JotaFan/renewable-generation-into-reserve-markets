...
def merge_predictions(results_df):
    test_name = "Tail_cluster_study_results_6"
    base_paths_cols = [f for f in results_df.columns if test_name in f]
    base_paths = [f.replace(f"{test_name}_", "").split("_model")[0] for f in base_paths_cols]
    base_paths = list(set(base_paths))
    new_df = pd.DataFrame()
    for base in base_paths:
        cols_base = [f for f in base_paths_cols if base in f]
        timerange = max([int(f.split(".keras")[0].split("_")[-1]) for f in cols_base]) + 1
        # Sort the columns by time
        new_cols_base = []
        for i in range(timerange):
            v = [f for f in cols_base if int(f.split(".keras")[0].split("_")[-1])==i][0]
            new_cols_base.append(v)

        values = []
        for i in range(len(results_df)):
            for col in new_cols_base:
                values.append(results_df.loc[i, col])
